...
export const PieceIcon: React.FC<PieceIconProps> = ({ type, color, className = 'w-full h-full' }) => {
  ...
  const strokeColor = color === 'w' ? '#5c3a21' : '#1a0d00';      // Medium wood brown vs deep black-brown
  const fillColor = color === 'w' ? '#e2c59d' : '#321f11';        // Natural birch/pine vs dark ebony/mahogany
  const secondaryColor = color === 'w' ? '#ebd4b7' : '#462c19';  // Lighter cream highlights vs dark warm chocolate

  // Custom SVGs representing Ouk Chatrang traditional wooden figures (from the image)
  switch (type) {
